...
```{r}
# install Bioconductor if missing ggtree for harrietr
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")

# Then install ggtree through Bioconductor
...
inter_distance <- split_res %>% group_by(Species, Species2) %>% summarise(mean(JC69))
```
